...
float falloff(vec2 uv, vec2 src, float aspect, float r, float sq) {
  ...
  float ang = 0.35; float ca = cos(ang), sa = sin(ang);
...
void main() {
  ...
  float light = max(
    falloff(uv, vec2(0.20, 0.10), aspect, 1.20, 0.28),
    falloff(uv, vec2(0.90, 0.92), aspect, 0.85, 0.40) * 0.40
  ) * u_reveal;

  float slatT    = fract(uv.x * u_blinds);
  float gap      = smoothstep(0.0, 0.04, slatT);
  float rim      = (1.0 - smoothstep(0.88, 1.0, slatT)) * light * 0.70;
  float face     = slatT * 0.65 * light;
  float gapFloor = mix(0.0, 0.04, light);
  float b        = ((face + rim) * mix(gapFloor, 1.0, gap)) * 1.05;

  float cx  = exp(-pow((uv.x - 0.50) * 2.2, 2.0) * 2.0);
  vec3  col = ramp(clamp(b, 0.0, 1.0)) * (1.0 - cx * 0.65);
